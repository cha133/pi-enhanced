...
export function mergeVisionRoute(
	globalValue: unknown,
	...
	const read = (value: unknown, source: string): Record<string, unknown> | undefined => {
		...
		if (!isRecord(value)) throw new Error(`The "vision" setting in ${source} must be a JSON object.`);
		return value;
	};
	...
	if (!globalRoute && !projectRoute) {
		if (options.required) throw new Error("vision model is not configured.");
		return undefined;
	}
...
export function loadVisionRoute(ctx: ExtensionContext, required: boolean): ModelRoute | undefined {
	...
	return mergeVisionRoute(globalSettings.vision, projectSettings.vision, {
		required,
