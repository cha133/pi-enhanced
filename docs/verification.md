...
- [x] 单元测试覆盖静态名称/hint 目录、关键词权重、分页浏览、精确 schema、超大单定义完整返回、跨 server 故障隔离和取消等待。
- [x] 单元测试覆盖缺失 hint 生成、全局/项目来源写回、保留其他配置、手工 hint/endpoint 变动、部分失败继续和取消保留成功项。
- [x] 模拟命令测试覆盖当前模型独立请求、usage 记录、完成通知和落盘；所有测试无需凭据或外部模型。
- [ ] 在真实 Pi TUI 中目视验收 `/mcp-gen-hints` 的进度/Esc 取消及真实模型生成；本次不自动修改用户 MCP 配置。
